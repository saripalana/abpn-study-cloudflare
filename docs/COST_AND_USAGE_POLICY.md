...
3. **Cloudflare D1 on Workers Free** for encrypted synchronization records.
...
- Maximum synchronization API requests: **2,000 per UTC day**
...
- No unauthenticated read endpoint containing study data
- Record-level synchronization only; never replace the complete database blindly

If an internal limit is reached, the app must stop cloud operations and continue in local-only mode.

## 5. Required emergency controls

Production must include:

1. A server-side `CLOUD_SYNC_ENABLED` kill switch. Any value other than the explicit enabled value must reject cloud writes.
...
- Review it again after the first successful multi-device synchronization
...
5. Confirm the app remains usable locally.
