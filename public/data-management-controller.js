...
async function discardActiveSet(bank) {
  ...
  if (!set) {
    alert("No active set was found for this question bank.");
    location.reload();
    return;
  }
...
async function resetCurrentBank(bank) {
  const firstConfirmation = confirm([
    ...
    "",
    "Question-bank content, other banks, downloaded backups, and Cloudflare data will not be deleted.",
...
async function attachControls() {
  ...
  try {
    ...
    if (protectionActions && !protectionActions.querySelector("#resetBankBtn")) {
      ...
      resetButton.textContent = "Reset current bank";
    ...
    if (protectionSection && !protectionSection.querySelector(".data-management-note")) {
      ...
      note.textContent = "Destructive actions are limited to the selected question bank and create a recovery snapshot before deletion.";
